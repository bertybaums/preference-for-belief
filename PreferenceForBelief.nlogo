;;shared ideological preference: 58-60 for moderate, 65 for liberal/conserve, 72 for extreme
;;25% very lib/con, 40% lib/con, 35% mod
;; unmotivated and motivated belief spread, but no "reflection" or "fact-checking"
...
to belief-spread
  ask turtles with [belief != 0] [
    ask link-neighbors with [belief = 0] [
      if ideology-level = 0 [;moderates use the baseline level set by the motivated-bias global variable
        ifelse random-float 1 < motivated-bias [
          if color = orange and [belief] of myself = 1 [set belief 1]
          if color = violet and [belief] of myself = 2 [set belief 2]
        ]
        [
